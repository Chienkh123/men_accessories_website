...
function check_product($id_product, $size, $quantity_order) {
  ...
  if ($result && mysqli_num_rows($result) > 0) {
      ...
      $size_product = unserialize($row['size_product']); // Sử dụng 'size_product' từ $row
      // Kiểm tra xem kích thước có tồn tại không và số lượng đặt hàng có lớn hơn số lượng còn lại không
      if (isset($size_product[strtolower($size)]) && $size_product[strtolower($size)] < $quantity_order) {
          return true; // Không đủ hàng
      }
  }
  return false; // Sản phẩm không tồn tại hoặc có đủ hàng
}
